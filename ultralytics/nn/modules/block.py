...
class LearnableHighPass(nn.Module):
    ...
    def __init__(self, dim):
        super().__init__()
        # 初始化为 1.0，表示默认进行完全的减法，但允许网络调整
        self.alpha = nn.Parameter(torch.ones(1, dim, 1, 1) * 1.0)
        self.avg_pool = nn.AvgPool2d(kernel_size=3, stride=1, padding=1)

    def forward(self, x):
        low_freq = self.avg_pool(x)
        # 核心公式：X_high = X - alpha * X_low
        high_freq = x - self.alpha * low_freq
        return high_freq

class LAHF(nn.Module):
    expansion = 1
    ...
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        ...
        # 1. 投影 + 下采样
        self.proj = ConvNormLayer(ch_in, ch_out, kernel_size=1, stride=stride, act=None)

        # 2. 高低频通道拆分
        self.c_h = ch_out // 2
        self.c_l = ch_out - self.c_h

        # ---- [核心修改] 高频分支 ----
        # 使用可学习的高通滤波代替原来的直接减法
        self.lhpf = LearnableHighPass(self.c_h)

        self.dw_h = nn.Conv2d(self.c_h, self.c_h, 3, 1, 1, groups=self.c_h, bias=False)
        self.bn_h = nn.BatchNorm2d(self.c_h)

        # ---- 低频分支 (保持不变) ----
        self.dw_l = nn.Conv2d(self.c_l, self.c_l, 5, 1, 2, groups=self.c_l, bias=False)
        self.bn_l = nn.BatchNorm2d(self.c_l)

        # 3. 融合
        self.fuse = ConvNormLayer(ch_out, ch_out, kernel_size=1, stride=1, act=None)

        # 4. 门控 (SE-like)
        reduction = 4
        hidden = max(1, ch_out // reduction)
        self.gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(ch_out, hidden, 1, 1, 0, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(hidden, ch_out, 1, 1, 0, bias=True),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        x_proj = self.proj(x)
        x_h, x_l = torch.split(x_proj, [self.c_h, self.c_l], dim=1)
...
import torch.nn.functional as F
DFEB = LAHF
class HFConcatGate(nn.Module):
    """
    High-Frequency Energy Guided Concat (HFE-Gated Concat)
    输入: x = [x1, x2] (list/tuple), 且 x1/x2 的 H,W 相同
    输出: cat([x1*w1, x2*w2], dim=1)，通道数保持为 C1+C2
    参数量: 1 个标量 raw_tau（每个模块实例）
    """
    def __init__(self, k=3, init_tau=2.0, min_w=0.5, eps=1e-6):
        super().__init__()
        self.pool = nn.AvgPool2d(kernel_size=k, stride=1, padding=k // 2)
        self.raw_tau = nn.Parameter(torch.tensor(float(init_tau)))
        self.min_w = float(min_w)
        self.eps = eps

    def _hf_energy(self, x):
        hf = x - self.pool(x)
        # (B,1,H,W)
        return hf.abs().mean(dim=1, keepdim=True)

    def forward(self, x):
        assert isinstance(x, (list, tuple)) and len(x) == 2, "HFConcatGate expects [x1, x2]"
        x1, x2 = x

        e1 = self._hf_energy(x1)
        e2 = self._hf_energy(x2)

        tau = F.softplus(self.raw_tau) + self.eps  # 保证 tau>0
        g = torch.sigmoid(tau * (e2 - e1))         # e2更强 => 更偏向x2

        w2 = self.min_w + (1.0 - self.min_w) * g
        w1 = self.min_w + (1.0 - self.min_w) * (1.0 - g)
