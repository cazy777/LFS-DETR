...
from ultralytics.nn.detr_modules.blocks import *
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    import ast
    def _cin(f_index):
        return sum(ch[i] for i in f_index) if isinstance(f_index, (list, tuple)) else ch[f_index]
    # Args
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        # === 新增：FAMix_Lite ===
            ...
            # 允许两种写法：
            # 1) args = [split_ratio, dilations, e_lambda, ...]
            # 2) args = [in_channels, split_ratio, dilations, e_lambda, ...]
            if len(args) == 0:
                args = [in_ch]
            else:
                if isinstance(args[0], int):
                    # 显式给了 in_channels
                    if args[0] != in_ch:
                        raise ValueError(f"SAFM in_channels mismatch: yaml={args[0]} vs inferred={in_ch}")
                    # args 保持不变
                else:
                    # 未显式给 in_channels（float/list/tuple 等超参开头）
                    args = [in_ch, *args]
        ...
        # elif m is AFDS:
        #     in_ch = _cin(f)
        #     # 期望构造: (in_channels, out_channels)
        #     if len(args) == 0:
        #         out_ch = in_ch
        #         args = [in_ch, out_ch]
        #     elif len(args) == 1:
        #         out_ch = make_divisible(args[0], 8)
        #         args = [in_ch, out_ch]
        #     else:
        #         # 若 YAML 已给 in_channels(可忽略)/out_channels，统一为 [in_ch, out_ch]
        #         out_ch = make_divisible(args[1], 8) if isinstance(args[1], (int, float)) else _cin(f)
        #         args = [in_ch, out_ch]
        #     c2 = out_ch  # 该层输出通道

        # === 新增：GAU ===
        # elif m is SAGM:
        #     # 期望 from=[i, j]，构造: (ch_in1, ch_in2, out_channels)
        #     assert isinstance(f, (list, tuple)) and len(f) >= 2, "GAU expects from=[i,j]"
        #     ch1 = ch[f[0]]
        #     ch2_in = ch[f[1]]
        #     if len(args) >= 3:
        #         out_ch = make_divisible(args[2], 8)
        #     else:
        #         out_ch = make_divisible(ch1, 8)  # 默认与高分辨率分支一致
        #     args = [ch1, ch2_in, out_ch]
        #     c2 = out_ch
            ...
            assert isinstance(f, (list, tuple)) and len(f) == 2, "CSAF expects from=[i,j]"
            c1 = ch[f[0]]
            c2_ = ch[f[1]]
            assert c1 == c2_, f"CSAF expects equal channels, got {c1} and {c2_}"
            # 输出是 concat 后的 2C
            c2 = c1 + c2_
            # args = [r, k, tau] -> __init__(c, r, k, tau)
            args = [c1, *args]
